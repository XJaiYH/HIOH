...
def boost(data_ori:np.ndarray, data: np.ndarray, data_idx, previous_map, args, density, radius)->np.ndarray:
    ...
    # if args.verbose:
    #     print(prompt + "check whether achieving the ending conditions")
    is_over = True
    for i in range(nn):
        j = 1
    ...
    if args.show_fig:
        ...
        if dd > 2:
            ...
            # plotCircle(data_ori, label, radius)
    ...
    for i in range(nn):
        ...
        for item in id_list:
            tmp_dis = np.sum(
                np.sqrt(np.sum(np.square(data_tmp[item] - data_tmp[id_list]), axis=1)))
            # tmp_dis = 0
            # for jj in range(len(id_list)):
            #     tmp_dis += distance_matrix[item, jj]
            if tmp_dis < min_dis:
...
if __name__=="__main__":
    ...
    for kk in k_set:
        args.k = kk
        print("****************************** # the coefficient is {} # ******************************".format(args.k))
        total_time = 0
        start = time.time()
        tree = KDTree(data)
        ...
        density = np.zeros((nn,))
        for i in range(nn):
            density[i] = np.exp(-radius_mean[i]) + random.random() / 1e6
        ...
        while True:
            ...
            if args.save_tmp_file:
                ...
                np.savetxt(args.save_dir + "/" + args.data_name + "/ameliorate-" + str(itr) + ".txt", data_tmp_save)

        total_time = time.time() - start
        # print(prompt + " after boosting!")
