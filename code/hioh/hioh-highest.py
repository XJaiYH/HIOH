...
def plotHotGraph(data, label):
    ...
    X, Y = np.meshgrid(x, y)
    total_C = np.zeros(X.shape)
    # for item in idx:
    #     iii = np.argwhere(label == item).reshape(-1, ).astype(int)
    #     data_demo = data[iii]
    #     points = np.empty((X.shape[0], X.shape[1], 2))
    #     points[:, :, 0] = X
    #     points[:, :, 1] = Y
    #     mu = np.mean(data_demo, axis=0)
    #     cov = np.cov(data_demo[:, 0], data_demo[:, 1])
    #     print()
    #     C = multivariate_normal.pdf(points, mean=mu, cov=cov)
    #     total_C += C
    #     print(C.shape)
    # cs = ax.contourf(X, Y, total_C, locator=ticker.LogLocator(), cmap=plt.get_cmap('Spectral'))
    # cbar = fig.colorbar(cs, fraction=0.1, pad=0.15, shrink=0.9, anchor=(0.0, 0.3))  # 对colorbar的大小进行设置
    density = ax.scatter_density(data[:, 0], data[:, 1], cmap='Spectral_r')
    cbar = fig.colorbar(density, label="the density of each pixel")
    tick_locator = ticker.MaxNLocator(nbins=6)
    cbar.locator = tick_locator
    cbar.ax.tick_params(labelsize=12.5)
    cbar.ax.set_title('density', fontsize=12.5)
...
def boost(data_ori:np.ndarray, data: np.ndarray, data_idx, previous_map, args, density, radius)->np.ndarray:
    ...
    for i in range(nn):
        j = 1
        density_max = density[data_idx[i]]
        density_max_idx = -1
        while j < min(args.k+1, nn) and distance_sort[i, j] <= radius[data_idx[i]]:
            if density_max < density[data_idx[sort_idx[i, j]]]:
                density_max = density[data_idx[sort_idx[i, j]]]
                density_max_idx = sort_idx[i, j]
            j += 1
        if density_max_idx != -1:
            if hashmap.get(data_idx[density_max_idx]) is None:
                hashmap[data_idx[density_max_idx]] = []
            if visit[data_idx[i]] is False:
                visit[data_idx[i]] = True
                hashmap[data_idx[density_max_idx]].append(int(data_idx[i]))
            hashmap[data_idx[density_max_idx]].extend(previous_map.get(data_idx[i], []))
        else:
...
if __name__=="__main__":
    ...
    parser.add_argument('--data_name', default='letter.csv', type=str,
    ...
    algo_name = 'DM-averaging-fuzz-highest-formal'
    ...
    for kk in k_set:
        ...
        itr = 1
        while True:
            # print(prompt + "************** the {}-th iteration **************".format(itr))
            itr += 1
            # print(prompt + "in boost function")
            start = time.time()
        ...
        # distance_copy = pdist(data_boost)
        # distance_copy_matrix = squareform(distance_copy)
        # distance_copy_sort = np.sort(distance_copy_matrix, axis=1)
        # r_base = np.mean(distance_copy_sort[:, 1])
