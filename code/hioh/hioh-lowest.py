...
def boost(data_ori:np.ndarray, data: np.ndarray, data_idx, previous_map, args, density, radius)->np.ndarray:
    ...
    for i in range(nn):
        j = 1
        density_lowest_idx = -1
        density_lowest = 999999
        while j < min(args.k+1, nn) and distance_sort[i, j] <= radius[data_idx[i]]:
            if density[data_idx[sort_idx[i, j]]] > density[data_idx[i]] and density[data_idx[sort_idx[i, j]]] < density_lowest:
                density_lowest_idx = sort_idx[i, j]
                density_lowest = density[data_idx[sort_idx[i, j]]]
            j += 1
        if density_lowest_idx != -1:
            if hashmap.get(data_idx[density_lowest_idx]) is None:
                hashmap[data_idx[density_lowest_idx]] = []
            if visit[data_idx[i]] is False:
                visit[data_idx[i]] = True
                hashmap[data_idx[density_lowest_idx]].append(int(data_idx[i]))
            hashmap[data_idx[density_lowest_idx]].extend(previous_map.get(data_idx[i], []))
        else:
...
if __name__=="__main__":
    ...
    algo_name = 'DM-averaging-fuzz-lowest-formal'
