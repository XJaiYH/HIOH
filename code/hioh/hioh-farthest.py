...
def boost(data_ori:np.ndarray, data: np.ndarray, data_idx, previous_map, args, density, radius)->np.ndarray:
    ...
    for i in range(nn):
        j = 1
        density_farthest_idx = -1
        while j < min(args.k+1, nn) and distance_sort[i, j] <= radius[data_idx[i]]:
            if density[data_idx[sort_idx[i, j]]] > density[data_idx[i]]:
                density_farthest_idx = sort_idx[i, j]
            j += 1
        if density_farthest_idx != -1:
            if hashmap.get(data_idx[density_farthest_idx]) is None:
                hashmap[data_idx[density_farthest_idx]] = []
            if visit[data_idx[i]] is False:
                visit[data_idx[i]] = True
                hashmap[data_idx[density_farthest_idx]].append(int(data_idx[i]))
            hashmap[data_idx[density_farthest_idx]].extend(previous_map.get(data_idx[i], []))
        else:
...
if __name__=="__main__":
    ...
    algo_name = 'DM-averaging-fuzz-farthest-formal'
